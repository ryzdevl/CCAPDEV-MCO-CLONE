$(document).ready(function() {
    ...
    function loadOtherUserProfile(userId) {
        if (!TLPostLayout.template) {
            TLPostLayout.init().then(function() {
                loadOtherUserProfile(userId); // initializing posts
            });
            return;
        }
    ...
    function setupFollowButton(otherUser) {
        ...
        const isFollowing = window.currentUser.following && 
                           window.currentUser.following.includes(otherUser._id);
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        if (response.following) {
                            ...
                            window.currentUser.following = window.currentUser.following.filter(id => id !== otherUser._id);
                        }
    ...
    // Function to display highlights in the highlights section
    ...
    // Function to load highlights (for own profile)
    ...
    // BTW refreshHighlights will b called by TLPostLayout so it can render all user highlights for respective logged in users
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if(response.success && response.data) {
                    ...
                    displayPosts(userPosts);
                    loadHighlights(); // Load highlights after posts are loaded
                } else {
    ...
    function displayPosts(posts) {
        ...
        postsContainer.html('<h3 id="post-header">User posts</h3>');
    ...
    $("#gallery-modal-btn").click(function(e){
       ...
       // Determine which user's gallery to show
        ...
        // Pass the userId to the gallery page
